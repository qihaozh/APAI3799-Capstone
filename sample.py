"""
Sample: 使用 Gemini 2.0 Flash 运行医疗对话模拟
==============================================
运行前请确保:
  1. pip install google-genai
  2. 设置你的 Google API Key（下方直接填入或设置环境变量 GOOGLE_API_KEY）
"""
...
# 预览一条记录的基本信息
rec = records[3]
print(f"📋 选中记录 ID={rec.record_id}")
print(f"   疾病: {rec.disease}")
print(f"   医院: {rec.hospital} - {rec.department}")
print(f"   真实对话轮数: {len(rec.dialogue)}\n")

# ──────────────────────────────────────────────
# 3. 自动模拟对话 (AI 同时扮演医生和患者)
# ──────────────────────────────────────────────
print("🔄 正在使用 Gemini 3.0 Flash 模拟对话...\n")
